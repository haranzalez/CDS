...
class DocumentUpload
{
	...
	public function __construct($uploadFolder, $cid)
	{
		...
		$this->destination .= $this->clientId;
	...
	protected function getErrorMessage($file)
	{
		switch($file['error']) {
			case 1:
			case 2:
				$this->messages[] = 'm3 ' . $file['name'] . ' is too big: (max: ' . 
				self::convertFromBytes($this->maxSize) . ').';
				break;
			case 3:
				$this->messages[] = 'm3 ' . $file['name'] . ' was only partially uploaded.';
				break;
			case 4:
				$this->messages[] = 'm3 No file submitted.';
				break;
			default:
				$this->messages[] = 'm4 Sorry, there was a problem uploading ' . $file['name'];
				break;
		}
	}
	...
	{
		if ($file['size'] == 0) {
			$this->messages[] = 'm3' . $file['name'] . ' is empty.';
			return false;
		} elseif ($file['size'] > $this->maxSize) {
			$this->messages[] = 'm2' . $file['name'] . ' exceeds the maximum size for a file ('
					. self::convertFromBytes($this->maxSize) . ').';
			return false;
		} else {
			return true;
	...
	protected function checkType($file) 
	{
		if (in_array($file['type'], $this->permittedTypes)) {
			...
			$this->messages[] = 'm2' . $file['name'] . ' is not permitted type of file.';
			return false;
		}
	}
	...
	protected function checkName($file)
	{
		...
		$nospaces = $this->clientId .= '_'.$nospaces;
	...
	protected function moveFile($file)
	{
		...
		if ($success) {
			$result = 'm1 ' . $file['name'] . ' was uploaded successfully';
			...
			$this->messages[] = 'm4 Could not upload ' . $file['name'];
		}
	}
